...
def compute_distance_matrix(summaries: torch.Tensor) -> np.ndarray:
    """Compute squared 2-Wasserstein distance for diagonal Gaussians.

    The summary stores diagonal variances. For diagonal covariance matrices,
    the Bures/Wasserstein covariance term is the squared Euclidean distance
    between standard deviations, not between variances.
    ...
    var = summaries[:, D_ACTION:]   # (N, d)
    std = torch.sqrt(torch.clamp(var, min=0.0))

    # ||mu_i - mu_j||^2
    mu_dist = torch.cdist(mu, mu, p=2).pow(2)
    # ||sqrt(var_i) - sqrt(var_j)||^2
    cov_dist = torch.cdist(std, std, p=2).pow(2)

    dist = (mu_dist + cov_dist).numpy()
