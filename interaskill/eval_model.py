...
from .api_models import APIClient, is_api_model, parse_api_model, api_model_short_name
from .paths import (
    BROWSECOMP_CONVERSATIONS,
    IW_VAL_CONVERSATIONS,
    RESULTS_DIR,
    WEBARENA_CONVERSATIONS,
    WORKARENA_CONVERSATIONS,
    WORKARENA_TRAJECTORIES,
)
...
# Format-constraint instruction appended to the system prompt for zero-shot
# evaluation. LoRA-tuned models learn this format from training data and don't
# need the explicit instruction; for zero-shot models (closed-source or
# instruction-tuned open-source), we add this to level the comparison.
FORMAT_CONSTRAINT = (
    "\n\nRESPONSE FORMAT (STRICT): Respond with exactly one line in the format "
    "[Action: skill_name] where skill_name is one of: {skills}. "
    "Output only that single line. No explanations, no preamble, no extra text."
)


def apply_format_constraint(messages: list[dict]) -> list[dict]:
    """Append a strict [Action: skill_name] format instruction to the system
    message. Returns a new list; does not modify the input.

    If no system message exists, inserts one at the start.
    """
    skills = ", ".join(sorted(VALID_SKILLS))
    instruction = FORMAT_CONSTRAINT.format(skills=skills)
    ...
    if not injected:
        out.insert(0, {"role": "system",
                       "content": "You are a skill classifier." + instruction})
    return out
...
def parse_args():
    ...
    parser.add_argument("--dataset", choices=["iw", "wa", "bc", "workarena"],
                        default="iw",
                        help="Dataset: iw (fabricated), wa (WebArena), "
                        "bc (BrowseComp-Plus), or workarena")
    ...
    parser.add_argument("--format-constraint", action="store_true",
                        help="Append strict [Action: skill_name] format "
                        "instruction to the system prompt. Use for fair "
                        "zero-shot comparison; unnecessary for LoRA models.")
...
def load_model_and_tokenizer(model_name: str, adapter_path: str = None):
    ...
    If model_name starts with "api:", returns (APIClient, None) — no HF load.
    """
    # Helpful error for local-path model names that don't exist.
    if model_name.startswith("/") or model_name.startswith("./") or model_name.startswith("../"):
        if not Path(model_name).exists():
            raise FileNotFoundError(
                ...
                "If this is a PPO/full-model checkpoint, make sure training has finished "
                "and `final_model/` exists."
            )

    if is_api_model(model_name):
        if adapter_path:
            raise ValueError("Cannot attach a LoRA adapter to an API model")
        provider, model_id = parse_api_model(model_name)
        print(f"Using API model: provider={provider}, model={model_id}")
        return APIClient(provider, model_id), None
...
def generate_response(model, tokenizer, messages: list[dict],
    ...
    if isinstance(model, APIClient):
        return model.call(messages, max_new_tokens=max_new_tokens)
    try:
...
def model_short_name(model_name: str) -> str:
    ...
    # api:anthropic-foundry/claude-sonnet-4-5 -> claude-sonnet-4-5
    # api:openai/gpt-5 -> gpt-5
    if is_api_model(model_name):
        return api_model_short_name(model_name)
...
def _checkpoint_path(short_name: str, dataset: str) -> Path:
    suffix = f"_{dataset}" if dataset in ("wa", "bc", "workarena") else ""
...
def main():
    ...
    if args.dataset == "wa":
        ...
        val_path = BROWSECOMP_CONVERSATIONS
    elif args.dataset == "workarena":
        val_path = WORKARENA_CONVERSATIONS
    else:
        val_path = IW_VAL_CONVERSATIONS

    if not val_path.exists():
        raise FileNotFoundError(
            f"Missing evaluation data: {val_path}\n"
            "For WorkArena, generate it with:\n"
            "  python scripts/data/generate_workarena_conversations.py "
            f"--input {WORKARENA_TRAJECTORIES}"
        )
    ...
    for ci, conv in enumerate(conversations):
        ...
        for mi in range(len(msgs)):
            ...
            context = msgs[:mi]
            if args.format_constraint:
                context = apply_format_constraint(context)
    ...
    # Save results
    suffix = f"_{args.dataset}" if args.dataset in ("wa", "bc", "workarena") else ""
    if args.adapter:
        short_name += "_lora"
    if args.format_constraint:
        short_name += "_fc"
