...
# ══════════════════════════════════════════════════════════════════════
# Baseline 1b: Auto-Generated SKILL.md (from pipeline)
# ══════════════════════════════════════════════════════════════════════
#
# Uses transition probabilities auto-generated by the InteraSkill pipeline
# (Phase 4: generate_skillmd.py). Same greedy prediction as SkillMdBaseline
# but transitions come from data-driven discovery, not manual engineering.

class SkillMdParsedBaseline:
    """Hand-crafted SKILL.md baseline that *parses real SKILL.md files*.

    Reviewer-rebuttal baseline: instead of a hard-coded Python dict, this
    reads the hand-authored SKILL.md files under skills/<skill>/SKILL.md
    and parses their "Transitions → To" probability tables. This is the
    fairest possible hand-crafted comparison — the same artifact a human
    engineer would write following the anthropics/skills convention.
    """

    def __init__(self, skills_dir: str = "skills", stochastic: bool = False):
        self.skills_dir = Path(skills_dir)
        self.stochastic = stochastic
        self.transitions = self._parse_all_skills()

    def _parse_all_skills(self) -> dict:
        """Read skills/<name>/SKILL.md, extract 'To' table under ## Transitions."""
        import re
        result = {}
        for skill_dir in sorted(self.skills_dir.iterdir()):
            ...
            result[skill_name] = self._parse_to_table(text, skill_name)
        # Fill missing entries with uniform distribution over all skills
        for s in SKILL_TYPES:
            if s not in result or not result[s]:
                result[s] = {t: 1.0 / N_SKILLS for t in SKILL_TYPES}
        return result

    @staticmethod
    def _parse_to_table(text: str, skill_name: str) -> dict:
        """Find the second '| To |' table within '## Transitions' section."""
        import re
        trans_match = re.search(r"##\s+Transitions\s*\n(.*?)(?=\n##\s)", text, re.DOTALL)
        if not trans_match:
            return {}
        ...
        if not to_match:
            return {}
        probs = {}
        for line in to_match.group(1).strip().split("\n"):
            line = line.strip()
            ...
            if len(cells) < 2:
                continue
            tgt_raw = cells[0].strip("`").strip()
            if tgt_raw in ("(end)", ""):
                continue
            if tgt_raw not in SKILL_TYPES:
                continue
            try:
                p = float(cells[1])
            except ValueError:
    ...
    def predict_sequence(self, first_skill: str, length: int) -> list[str]:
        ...
        current = first_skill
        for _ in range(length - 1):
            dist = self.transitions.get(current, {"document_edit": 1.0})
            if self.stochastic and dist:
                skills = list(dist.keys())
                ps = np.array(list(dist.values()), dtype=float)
                ps = ps / ps.sum()
                next_skill = str(np.random.choice(skills, p=ps))
            else:
                next_skill = max(dist, key=dist.get)
            seq.append(next_skill)
            current = next_skill
...
class AutoSkillMdBaseline:
    """Auto-generated skill transition table from pipeline output."""

    def __init__(self, transition_dict: dict):
        """
        Args:
            transition_dict: {skill: {next_skill: probability}} from
                             SkillMdGenerator.get_transition_dict()
        """
        self.transitions = transition_dict

    def predict_sequence(self, first_skill: str, length: int) -> list[str]:
        """Predict a skill sequence using auto-generated transitions."""
        seq = [first_skill]
        current = first_skill
        for _ in range(length - 1):
            dist = self.transitions.get(current, {"document_edit": 1.0})
...
def run_all_baselines(traj_data_list, extra_baselines=None) -> dict:
    """Run all baselines and return results dict.

    Args:
        traj_data_list: Trajectories to evaluate on.
        extra_baselines: Optional list of (name, baseline_instance) tuples.
            Each baseline must have a predict_sequence(first_skill, length) method.
    """
    print("=" * 60)
    ...
    # 4. Extra baselines (e.g. auto-generated SKILL.md)
    if extra_baselines:
        for name, baseline in extra_baselines:
            results[name] = evaluate_baseline(baseline, traj_data_list, name)
...
if __name__ == "__main__":
    ...
    traj_data = load_and_featurize(str(IW_FABRICATED_TRAJECTORIES))
