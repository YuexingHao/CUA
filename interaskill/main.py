...
from .paths import IW_FABRICATED_TRAJECTORIES, RESULTS_DIR

DATA_PATH = IW_FABRICATED_TRAJECTORIES
...
def main():
    ...
    # ── Phase 4 (Optional): Auto-Generate SKILL.md ──────────────
    if "--generate-skillmd" in sys.argv:
        from .generate_skillmd import SkillMdGenerator
        from .data import load_trajectories
        print(f"\n[4+/7] Phase 4: Auto-generating SKILL.md files...")
        raw_trajs = load_trajectories(str(DATA_PATH))
        gen = SkillMdGenerator(
            traj_data_list=traj_data,
            segments=segments,
            gt_labels=labels,
            cluster_labels=best_cl,
            raw_trajectories=raw_trajs,
        )
        gen_result = gen.generate_all(RESULTS_DIR / "auto_skills")
        print(f"  Generated SKILL.md for {len(gen_result['skills_generated'])} skills")
        print(f"  Output: {RESULTS_DIR / 'auto_skills'}/")
...
def plot_composition_comparison(mlp_pos, tf_pos, save_path):
    ...
    # Position 0 is the provided seed skill, not a predicted next skill.
    positions = [p for p in sorted(set(mlp_pos.keys()) | set(tf_pos.keys())) if p != 0]
    ...
    ax.set_xlabel("Next-skill prediction step", fontsize=28)
    ax.set_xticks(x)
    ax.set_xticklabels([f"Step {p + 1}" for p in positions], fontsize=24)
    ax.tick_params(axis="y", labelsize=24)
    ax.set_ylim(0, 1.1)
    ax.legend(fontsize=36, frameon=True, fancybox=True, shadow=True)

    # Value labels
    for bar, acc in zip(bars1, mlp_accs):
        if acc > 0.01:
            ax.text(bar.get_x() + bar.get_width()/2, bar.get_height() + 0.02,
                    f"{acc:.2f}", ha="center", va="bottom", fontsize=20, color="#64748b")
