...
def _ensure_benchmark_registered(benchmark: str) -> None:
    """Import BrowserGym plugins that register non-core benchmark envs."""
...
def _benchmark_env_ids(benchmark: str) -> list[str]:
    """Return BrowserGym environment IDs for a benchmark.

    WebArena and MiniWoB have historically used numeric IDs in this runner.
    WorkArena registers named ServiceNow tasks, so we read them from the Gym
    registry instead of constructing browsergym/workarena.{i}.
    """
    _ensure_benchmark_registered(benchmark)
    if benchmark == "workarena":
...
def _env_id_for_task(benchmark: str, task_idx: int) -> str:
    env_ids = _benchmark_env_ids(benchmark)
    if env_ids:
        if task_idx >= len(env_ids):
            raise IndexError(
                f"{benchmark} task index {task_idx} is out of range "
                f"for {len(env_ids)} registered tasks"
            )
        return env_ids[task_idx]
    return f"browsergym/{benchmark}.{task_idx}"
...
WORKARENA_DIRECT_TEMPLATE = """You are controlling ServiceNow in BrowserGym.
...
Choose the single next browser action that makes progress.

ServiceNow guidance:
- Use the left navigation/filter navigator to reach modules.
- If a search/filter input is available, fill it with the exact target module or application words, then press("Enter").
- After opening a filtered result, click the matching module/link once.
- Do not repeat an action if it did not change the page. Try a different field, press Enter, scroll, or wait only once.
- Avoid noop() unless the page is actively loading.
- Use only BrowserGym element ids shown in square brackets in the accessibility tree.
...
class InteraSkillAgent:
    ...
    def _is_repeated_action(self, action: str, repeats: int = 2) -> bool:
        """Return True if action repeats the latest actions."""
        if len(self.history) < repeats:
            return False
        recent = self.history[-repeats:]
        return all(h["action"] == action for h in recent)
...
What skill should be executed next?"""},
    ...
    def ground_action(self, skill: str, observation: str) -> str:
        ...
        if self.grounding_mode == "direct" and "service-now.com" in getattr(self, "current_url", ""):
            template = WORKARENA_DIRECT_TEMPLATE
        elif self.grounding_mode == "direct":
        ...
        action = extract_browsergym_action(response)
        if self._is_repeated_action(action):
            retry_prompt = prompt + (
                f"\n\nThe proposed action {action} repeats the previous actions "
                "and did not make progress. Choose a different valid action now."
            )
            response = self.generate_fn(
            ...
            retry_action = extract_browsergym_action(response)
    ...
    def act(self, obs: dict) -> str:
        ...
        self.current_url = obs.get("url", "")

        # Stage 1: Predict skill
        skill = "direct_action" if self.grounding_mode == "direct" else self.predict_skill(obs_text)
...
def run_batch(agent: InteraSkillAgent, benchmark: str = "webarena",
    ...
    for task_idx in range(start_task, start_task + max_tasks):
        env_id = _env_id_for_task(benchmark, task_idx)
        try:
        ...
        try:
            ...
            try:
                ...
                if verbose:
                    print(f"  Warning: failed to close task {task_idx}: {e}")
...
def parse_args():
    ...
    parser.add_argument("--grounding-mode", type=str, default="llm",
                        choices=["llm", "direct"])
    parser.add_argument("--max-obs-length", type=int, default=4000,
                        help="Maximum accessibility-tree characters to show "
                        "to the model")
...
def main():
    ...
    print(f"Max obs length: {args.max_obs_length}")
    ...
    if args.url and args.goal:
        ...
        # Single task
        env_id = _env_id_for_task(args.benchmark, args.task_id)
        print(f"\nSingle task: {env_id}")
