...
python -m interaskill.eval_model \
    --model "$MODEL" \
    --dataset iw --max-convs 50 --max-new-tokens 200
...
python -m interaskill.eval_model \
    --model "$MODEL" \
    --dataset wa --max-convs 200 --max-new-tokens 200
...
echo "IW:"
cat results/deepseek-r1_eval_metrics.json 2>/dev/null
echo ""
echo "WebArena:"
cat results/deepseek-r1_eval_metrics_wa.json 2>/dev/null
