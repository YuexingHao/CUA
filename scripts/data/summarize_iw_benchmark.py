...
from interaskill.paths import IW_PARSED_TRAJECTORIES, IW_SKILL_TEMPLATES, IW_SUMMARY, ensure_parent

INPUT = IW_PARSED_TRAJECTORIES
SUMMARY_OUT = IW_SUMMARY
TEMPLATES_OUT = IW_SKILL_TEMPLATES
...
def main():
    ...
    # 3. Save
    with open(ensure_parent(SUMMARY_OUT), "w") as f:
        json.dump(summaries, f, indent=2)
    print(f"Saved {len(summaries)} summaries to {SUMMARY_OUT}")

    with open(ensure_parent(TEMPLATES_OUT), "w") as f:
